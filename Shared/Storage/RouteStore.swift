...
final class RouteStore: @unchecked Sendable, ObservableObject {
    ...
    private func migrateSchema(db: OpaquePointer) {
        let existingColumns = columnNames(table: "saved_routes", db: db)
        let migrations: [(column: String, sql: String)] = [
            ("modified_at", "ALTER TABLE saved_routes ADD COLUMN modified_at TEXT"),
            ("cloudkit_record_id", "ALTER TABLE saved_routes ADD COLUMN cloudkit_record_id TEXT")
        ]
